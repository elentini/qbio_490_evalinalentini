...
```{r}
setwd("/home1/elentini/490_cluster/analysis_data")
...
```{r}
clinic <- read.csv("/home1/elentini/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 4.1}
plot(as.numeric(clinic$age_at_diagnosis),
     as.numeric(clinic$last_contact_days_to),
     main = "Age at Diagnosis vs. Last Contact (days)",
...
```{r Exercise 4.2}
boxplot(as.numeric(age_at_diagnosis) ~ vital_status, data = clinic,
...
```{r}
male_mask <- toupper(clinic$gender) == "MALE"
```
...
cleaned_clinic$age_category <- ifelse(
  ...
  ifelse(as.numeric(cleaned_clinic$age_at_diagnosis) < 50, "Middle", "Old")
)
```
...
The y-axis shows the estimated survival probability (percentage of patients still alive) over time.

# 2. What does the x axis represent?
The x-axis shows time in days since diagnosis (often converted to years when interpreting long survival).

# 3. What age group appears to have the greatest rate of survival?
The “Young” group (≤ 35 years) shows the highest survival probability over time.

# 4. What age group appears to have the worst rate of survival? 
The “Old” group (≥ 50 years) has the lowest survival probability curve, indicating worse outcomes.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
About 40–50% of “Old” patients survive to 15 years (depending on the curve estimate).
...
The KM plot displays the log-rank test p-value. It is typically < 0.05, meaning it is statistically significant.

This indicates that survival distributions differ by age category, i.e., age at diagnosis is significantly associated with differences in breast cancer survival.
